...
class Module:
    ...
    def load_state_dict(self, state_dict: 'OrderedDict[str, Tensor]',
        ...
        temp=state_dict
        d2 = OrderedDict([('layers.0.graph_conv.lin.weight', v) if k == 'layers.0.graph_conv.weight' else (k, v) for k, v in state_dict.items()])
#        print(d2)
   #     d3 = OrderedDict([('layers.0.graph_conv.lin.bias', v) if k == 'layers.0.graph_conv.bias' else (k, v) for k, v in d2.items()])
 #       print(d3)
        d4 = OrderedDict([('layers.1.graph_conv.lin.weight', v) if k == 'layers.1.graph_conv.weight' else (k, v) for k, v in d2.items()])
    #    print(d4)
  #      d5 = OrderedDict([('layers.1.graph_conv.lin.bias', v) if k == 'layers.1.graph_conv.bias' else (k, v) for k, v in d4.items()])
  #      print(d5)
        d6 = OrderedDict([('pool1.graph_pool.gnn.lin_rel.weight', v) if k == 'pool1.graph_pool.gnn.lin_l.weight' else (k, v) for k, v in d4.items()])
   #     print(d6)
        d7 = OrderedDict([('pool1.graph_pool.gnn.lin_rel.bias', v) if k == 'pool1.graph_pool.gnn.lin_l.bias' else (k, v) for k, v in d6.items()])
        #print(d7)
        d8 = OrderedDict([('pool1.graph_pool.gnn.lin_root.weight', v) if k == 'pool1.graph_pool.gnn.lin_r.weight' else (k, v) for k, v in d7.items()])
     #   print(d8)
        state_dict=d8
